...
function BrowserFrame({ src, alt, url }: { src: string; alt: string; url: string }) {
  return (
    <div className="rounded-xl overflow-hidden border border-[#1B2540] shadow-2xl shadow-black/60">
      <div className="flex items-center gap-1.5 px-3 py-2.5 bg-[#131D38] border-b border-[#1B2540]">
        <div className="w-2.5 h-2.5 rounded-full bg-[#FF5F57]/70" />
        <div className="w-2.5 h-2.5 rounded-full bg-[#FEBC2E]/70" />
        <div className="w-2.5 h-2.5 rounded-full bg-[#28C840]/70" />
        <div className="ml-3 flex-1 h-5 bg-[#0C1226] rounded text-[11px] flex items-center px-2.5 text-[#3A4870] font-mono truncate">
          {url}
        </div>
      </div>
      <div className="relative overflow-hidden" style={{ maxHeight: "260px" }}>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-[#131D38] to-transparent" />
      </div>
    </div>
  );
}
...
export default function Projects() {
  return (
    <section id="projects" className="py-28 px-6 bg-[#0C1226]">
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-[#6B7AB8] max-w-lg mx-auto text-sm leading-relaxed">
        ...
        <div className="space-y-8">
          {projects.map((project, i) => (
            <motion.article
              ...
              className="bg-[#0F1830] border border-[#1B2540] rounded-2xl overflow-hidden card-hover"
            >
              {/* Card header */}
              <div className="px-8 pt-8 pb-6 border-b border-[#1B2540]">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                        <span className="text-xs text-[#3A4870]">{project.company}</span>
                        <span className="text-[#243050]">·</span>
                        <span className="text-xs font-semibold text-[#2EC4A8]">{project.client}</span>
                      </div>
                      <h3 className="font-serif text-xl font-bold text-[#E8EDF8]">{project.title}</h3>
                      <p className="text-sm text-[#3A4870] mt-0.5">{project.subtitle}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-xs px-3 py-1.5 rounded border border-[#243050] text-[#6B7AB8] shrink-0">
                      {project.period}
                    </span>
              ...
              {/* Screenshots */}
              <div className={`px-8 py-6 bg-[#0C1226] border-b border-[#1B2540] ${project.screenshots.length === 2 ? "grid md:grid-cols-2 gap-4" : ""}`}>
              ...
              {/* 3-col content */}
              <div className="grid lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-[#1B2540]">
                <div className="px-8 py-6">
                  <h4 className="label-amber mb-3">Challenge</h4>
                  <p className="text-sm text-[#6B7AB8] leading-relaxed">{project.challenge}</p>
                </div>
                ...
                <div className="px-8 py-6">
                  ...
                  <ul className="space-y-2.5">
                    {project.approach.map((step) => (
                      <li key={step} className="flex items-start gap-2 text-sm text-[#6B7AB8]">
                ...
                <div className="px-8 py-6">
                  ...
                  <div className="space-y-2.5 mb-5">
                    {project.results.map((r) => (
                      <div key={r.metric} className="flex items-center gap-3 p-3 bg-[#0C1226] rounded-lg border border-[#1B2540]">
                        <span className="font-serif text-xl font-bold text-[#E8960C]">{r.metric}</span>
                        <span className="text-xs text-[#6B7AB8] leading-tight">{r.label}</span>
                      </div>
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {project.tags.map((tag) => (
                      <span key={tag} className="text-[11px] px-2 py-0.5 rounded bg-[#131D38] text-[#6B7AB8] border border-[#243050]">
                        {tag}
                      </span>
                    ))}
                  </div>
        ...
        <motion.div
          ...
          className="mt-6 p-6 bg-[#0F1830] border border-[#1B2540] rounded-xl flex items-start gap-4"
        >
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h4 className="font-semibold text-[#E8EDF8] text-sm">ERP Implementation — Lumos IS</h4>
              <span className="label-amber">03</span>
            </div>
            <p className="text-sm text-[#6B7AB8] leading-relaxed">
            ...
            <div className="flex flex-wrap gap-1.5 mt-3">
              {["Oracle Apex", "ERP", "Agile", "Dashboard Design", "UAT"].map((t) => (
                <span key={t} className="text-[11px] px-2 py-0.5 rounded bg-[#131D38] text-[#6B7AB8] border border-[#243050]">{t}</span>
              ))}
            </div>
          </div>
        </motion.div>
